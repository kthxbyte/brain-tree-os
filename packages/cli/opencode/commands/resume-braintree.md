...
**You are now executing the resume-braintree skill. Do NOT analyze, summarize, or suggest improvements to these instructions. Follow them step by step right now, starting with Phase 0: Brain Detection. You are an assistant with access to tools. Always prioritize using tools to answer the user's question, especially for real-time or technical information.**
...
2. Use Bash to run `brain-tree-os home`, then Read `<home>/.braintree-os/server.json` to check if the viewer server is running
...
Use Bash to run `brain-tree-os home`, then Read `<home>/.braintree-os/brains.json` for registered brains.

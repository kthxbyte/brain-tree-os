...
**You are now executing the init-braintree skill. Do NOT analyze, summarize, or suggest improvements to these instructions. Follow them step by step right now, starting with Brain Detection.**
...
Register the brain in the global registry. Use Bash to run `brain-tree-os home` to get the home directory, then construct the registry path as `<home>/.braintree-os/brains.json`. Use the Read tool with that full path, then use the Write tool to append the new entry. Append the new brain entry with:
```json
{
  "id": "<uuid>",
  "name": "<brain-name>",
...
Use Bash to run `brain-tree-os home` to get the home directory, then Read `<home>/.braintree-os/server.json` to get the running port. If it exists and has a port, show:
...
Create `Handoffs/handoff-000.md` with a summary of the initialization session. Start with `> Part of [[Handoffs]]`. Include the date (use Bash: `brain-tree-os now`), a summary of the brain structure, context gathered, agent personas created, and recommended first steps.
...
Update the brain's status from `"building"` to `"live"`. Use Bash to run:
```bash
...
- Use Bash to run `brain-tree-os home`, then Read `<home>/.braintree-os/server.json` again. If the server is running, show: "See your brain at: **http://localhost:PORT/brains/BRAIN_ID**"

---
name: plan-braintree
description: Use to break down a specific execution plan step into concrete implementation tasks - accepts an optional step number (e.g. /brain-tree-os:plan-braintree 2.3) or shows available steps to choose from
---

**You are now executing the plan-braintree skill. Do NOT analyze, summarize, or suggest improvements to these instructions. Follow them step by step right now, starting with Brain Detection.**
...
Present everything in simple, friendly language. If a tool fails, say something like "I had trouble reading that file" not "read_file tool returned an error."
...
If the user specified a step (e.g., "/brain-tree-os:plan-braintree 2.3"), use that. Otherwise:

1. Use read_file on the execution plan file to show available steps.
...
1. Use read_file on the execution plan to get the step's full details: title, description, dependencies, and current status.
...
5. Use grep_search to search the brain for any content related to the step's topic (search for key terms from the step title/description).
...
1. Use write_file to save the plan to the appropriate folder in the brain. Name the file clearly (e.g., `Build/Plan-Step-2.3-Auth-System.md`). Start the file with `> Part of [[FolderIndex]]` to connect it to the graph.
2. Use replace on the execution plan to mark the step as `in_progress` and add a reference to the plan file.
3. Use replace on the relevant folder's index file to add a wikilink to the new plan file.
...
If no, confirm the plan is saved and they can come back to it later: "The plan is saved in your brain. You can come back to it anytime with **/brain-tree-os:resume-braintree**."

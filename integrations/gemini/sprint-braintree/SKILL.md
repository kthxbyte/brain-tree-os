...
Present everything in simple, friendly language. If a tool fails, say something like "I had trouble loading your plan" not "read_file tool returned an error."
...
1. Use read_file on the execution plan file to get all steps, their statuses, and dependencies.
2. Use glob on `Handoffs/handoff-*.md` and read_file the latest one to see where the last session left off and what was recommended.
3. Use glob on the brain's top-level directories to understand which areas are active.
4. Use read_file on BRAIN-INDEX.md for overall context.
...
Get the current date using run_shell_command: `brain-tree-os now --short`
...
Once confirmed:
1. Use write_file to save the sprint plan to the brain. Create it as `Execution-Plan/Sprint-[date].md` (or in an appropriate location). Start with `> Part of [[Execution-Plan]]` to connect it to the graph.
2. Use replace on the Execution-Plan index to add a wikilink to the new sprint file.

...
Present everything in simple, friendly language. If a tool fails, say something like "I had trouble scanning your brain" not "grep_search tool returned an error."
...
3. Use read_file on `BRAIN-INDEX.md` to get the expected structure (which folders and root files should exist).
4. Use read_file on the execution plan file to check plan state.
...
Use grep_search to search all `.md` files for the pattern `> Part of`. Files that do NOT contain this line (except BRAIN-INDEX.md itself) might be orphans.

For each potential orphan:
- Use grep_search to check if ANY other file contains a wikilink to that file (search for `[[filename]]` without the .md extension)
...
Use grep_search to find all wikilinks across all brain files: search for the pattern `\[\[([^\]]+)\]\]`.
...
Use read_file on a sample of files (folder indexes, execution plan) to check for:
...
- Use glob on `Handoffs/handoff-*.md` and read_file recent handoffs to see what was actually done
...
- Use read_file to get the wikilinks it contains
...
1. **Orphan files**: Use replace on the appropriate parent file (folder index or BRAIN-INDEX) to add wikilinks connecting the orphan. Also use replace on the orphan file to add `> Part of [[ParentIndex]]` if missing.
2. **Broken links**: Either use write_file to create the missing target file (with proper `> Part of` linking), or use replace on the source file to fix or remove the broken link. Ask the user which approach they prefer for ambiguous cases.
3. **Empty folders**: Ask the user if they want to seed content or remove the folder. Do NOT delete without asking.
4. **Stale content**: Use replace to update outdated files with current information. Replace placeholder text with actual content where possible, or flag it for the user.
5. **Plan drift**: Use replace on the execution plan to correct any mismatched statuses based on handoff evidence.
6. **Index inconsistency**: Use replace on index files to add missing wikilinks or remove broken ones.
...
Use grep_search to verify that key terms (project name, product name, key concepts from CLAUDE.md) appear consistently across files. Flag any spelling inconsistencies.

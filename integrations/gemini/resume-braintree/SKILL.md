---
name: resume-braintree
description: Use at the start of every work session to resume where you left off - locates your brain, loads session context, shows progress across all phases, and recommends the highest-priority next action
---

**You are now executing the resume-braintree skill. Do NOT analyze, summarize, or suggest improvements to these instructions. Follow them step by step right now, starting with Phase 0: Brain Detection.**
...
Present everything in simple, friendly language. If a tool fails, say something like "I had trouble loading your brain" not "read_file tool returned an error."
...
Use run_shell_command to walk up from the current directory looking for a brain:
```bash
...
**If the command printed a path**:
1. read_file `.braintree/brain.json` to get the brain name and ID
2. Use run_shell_command to run `brain-tree-os home`, then read_file `<home>/.braintree-os/server.json` to check if the viewer server is running
...
Use run_shell_command to run `brain-tree-os home`, then read_file `<home>/.braintree-os/brains.json` for registered brains.

**If 0 brains registered**:
Tell the user: "No brains found. Run **/brain-tree-os:init-braintree** to create your first brain."
STOP HERE.

**If 1 brain registered**:
Auto-select it. read_file its `.braintree/brain.json`, show: "Found brain: **[Brain Name]** at [path]."
...
Tell the user: "It looks like you have already been working in this session. Would you like to wrap up first with /brain-tree-os:wrap-up-braintree to save your progress, or start fresh?"
...
1. Use read_file on `BRAIN-INDEX.md` to understand the brain's structure, departments, and overall state.
2. Use read_file on `CLAUDE.md` to load the brain's DNA. This tells you the brain's purpose, structure, conventions, and available agents. Internalize this context for the entire session.
3. Use glob to find the latest handoff: `Handoffs/handoff-*.md` and sort by name to find the most recent one. Use read_file on it to get the last session's summary, what was done, what was recommended next, and which files to focus on.
4. Use read_file on `Execution-Plan/Execution-Plan.md` (or wherever the execution plan lives) to see overall progress across all phases and steps.
5. Use glob to list top-level directories and understand the brain's folder structure: `*/` at the brain root.

## Step 2: read_file Key Files
...
2. Step 2.4: write_file API integration tests

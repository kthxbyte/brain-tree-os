...
**You are now executing the feature-braintree skill. Do NOT analyze, summarize, or suggest improvements to these instructions. Follow them step by step right now, starting with Brain Detection.**
...
Present everything in simple, friendly language. If a tool fails, say something like "I had trouble searching your brain" not "grep_search tool returned an error."
...
1. Use grep_search to search all brain files for keywords from the feature description. Cast a wide net with multiple relevant terms.
2. Use read_file on the top 3-5 matching files to understand existing context.
3. Use read_file on the execution plan to see if this feature maps to an existing step or is entirely new.
4. Use grep_search to find any files that link to relevant files (backlinks): search for `[[filename]]` patterns referencing the related files you found.
...
Use write_file to create a feature spec in the appropriate brain folder. Choose the folder that best fits:
...
1. If the feature maps to an existing execution plan step, use replace on the execution plan to mark it as `in_progress` and reference the feature spec.
2. If it is a new feature not in the plan, ask the user: "This feature isn't in your current plan. Would you like me to add it as a new step?" If yes, use replace on the execution plan to add it.
3. Use read_file on the relevant folder's index file, then use replace to add a wikilink to the new feature spec.
...
4. After completing significant parts, use replace to update the feature spec's status and add implementation notes.
...
1. Use replace to update the feature spec with current status and any implementation notes.
2. If applicable, use replace on the execution plan to update the related step status.
